...
const GameManager = (function () {
  ...
  function onEnemyHit(hit) {
    ...
    if (remaining <= 0) {
      ...
      if (Math.random() < 0.40) {
        ...
        if (!dropWeaponId) dropWeaponId = null; // null = no weapon drop for this type
  ...
  function update() {
    ...
    if (_perfCheckTimer > 2 && _fpsSamples > 8) {
      ...
      // Step UP: recover slowly when there's headroom, but never bring a phone back
      // below tier 1 (keeps shadows/env off — ULTRA settings re-introduce stutter).
