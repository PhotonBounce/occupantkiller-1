...
window.VoxelWorld = (function () {
  ...
  // ── ZAPORIZHZHIA NPP: Nuclear Power Plant ────────────────────────────────
  // Zaporizhzhia NPP is the largest nuclear plant in Europe. Russia seized it in March
  // 2022 — triggering the worst nuclear crisis since Chernobyl. Six VVER-1000 reactors.
  function generateZaporizhzhiaNPP(ox, oz) {
    var h = getHeight(ox, oz);
    var x, z, i, j;
    // 6 reactor buildings in 2 rows of 3
    var reactorPositions = [[-22,0],[-8,0],[6,0],[-22,15],[-8,15],[6,15]];
    for (j = 0; j < reactorPositions.length; j++) {
      var rx = ox + reactorPositions[j][0], rz = oz + reactorPositions[j][1];
      var rh = getHeight(rx, rz);
      // 8×8 CONCRETE cube 12 high
      for (x = rx; x < rx + 8; x++) {
        for (z = rz; z < rz + 8; z++) {
          var isEdgeR = (x === rx || x === rx + 7 || z === rz || z === rz + 7);
          for (i = 0; i < 12; i++) {
            if (isEdgeR || i === 0) setBlock(x, rh + i, z, BLOCK.CONCRETE);
          }
        }
      }
      // Dome: CONCRETE pyramid 5×5×3 on top of cube (centered at rx+3, rz+3)
      for (i = 0; i < 3; i++) {
        var spread = 2 - i;
        for (x = rx + 3 - spread; x <= rx + 3 + spread; x++) {
          for (z = rz + 3 - spread; z <= rz + 3 + spread; z++) {
            setBlock(x, rh + 12 + i, z, BLOCK.CONCRETE);
          }
        }
      }
    }
    // 2 cooling towers: CONCRETE columns 3×3 base tapering to 2×2 at top, height 20
    var coolPositions = [[25,0],[25,15]];
    for (j = 0; j < coolPositions.length; j++) {
      var ctx = ox + coolPositions[j][0], ctz = oz + coolPositions[j][1];
      ...
      for (i = 0; i < 20; i++) {
        if (i < 10) {
          ...
          for (x = ctx; x <= ctx + 2; x++) {
            for (z = ctz; z <= ctz + 2; z++) {
              var onEdge3 = (x === ctx || x === ctx + 2 || z === ctz || z === ctz + 2);
              if (onEdge3) setBlock(x, cth + i, z, BLOCK.CONCRETE);
            }
          }
        } else {
          // 2×2 solid top
          setBlock(ctx, cth + i, ctz, BLOCK.CONCRETE);
          setBlock(ctx + 1, cth + i, ctz, BLOCK.CONCRETE);
          setBlock(ctx, cth + i, ctz + 1, BLOCK.CONCRETE);
          setBlock(ctx + 1, cth + i, ctz + 1, BLOCK.CONCRETE);
        }
      }
    }
    // Perimeter security fence: 70×45 rectangle METAL posts + FENCE at y+1
    var fw = 35, fd = 22;
    for (i = -fw; i <= fw; i++) {
      setBlock(ox + i, h, oz - fd, BLOCK.METAL);
      setBlock(ox + i, h + 1, oz - fd, BLOCK.FENCE);
      setBlock(ox + i, h, oz + fd, BLOCK.METAL);
      setBlock(ox + i, h + 1, oz + fd, BLOCK.FENCE);
    }
    for (i = -fd; i <= fd; i++) {
      setBlock(ox - fw, h, oz + i, BLOCK.METAL);
      setBlock(ox - fw, h + 1, oz + i, BLOCK.FENCE);
      setBlock(ox + fw, h, oz + i, BLOCK.METAL);
      setBlock(ox + fw, h + 1, oz + i, BLOCK.FENCE);
    }
    // Control building: 18×10 CONCRETE 8 high with GLASS windows, ROOFTILE flat roof
    var cbx = ox + 15, cbz = oz - 12;
    var cbh = getHeight(cbx, cbz);
    for (x = cbx - 9; x <= cbx + 9; x++) {
      for (z = cbz - 5; z <= cbz + 5; z++) {
        var isEdgeCB = (x === cbx - 9 || x === cbx + 9 || z === cbz - 5 || z === cbz + 5);
        if (isEdgeCB) {
          for (i = 0; i < 8; i++) setBlock(x, cbh + i, z, BLOCK.CONCRETE);
        } else {
          setBlock(x, cbh, z, BLOCK.CONCRETE);
        }
      }
    }
    // ROOFTILE flat roof
    for (x = cbx - 9; x <= cbx + 9; x++) {
      for (z = cbz - 5; z <= cbz + 5; z++) {
        setBlock(x, cbh + 8, z, BLOCK.ROOFTILE);
      }
    }
    // GLASS windows on facade every 3 blocks, 2 high
    for (x = cbx - 9; x <= cbx + 9; x += 3) {
      setBlock(x, cbh + 3, cbz - 5, BLOCK.GLASS);
      setBlock(x, cbh + 4, cbz - 5, BLOCK.GLASS);
      setBlock(x, cbh + 3, cbz + 5, BLOCK.GLASS);
      setBlock(x, cbh + 4, cbz + 5, BLOCK.GLASS);
    }
    // Emergency generators cluster: 4× METAL boxes (3×2 METAL, 2 high) near control building
    var genPos = [[cbx - 12, cbz - 8],[cbx - 9, cbz - 8],[cbx - 12, cbz - 11],[cbx - 9, cbz - 11]];
    for (j = 0; j < genPos.length; j++) {
      var gx = genPos[j][0], gz = genPos[j][1];
      var gh = getHeight(gx, gz);
      for (x = gx; x < gx + 3; x++) {
        for (z = gz; z < gz + 2; z++) {
          setBlock(x, gh, z, BLOCK.METAL);
          setBlock(x, gh + 1, z, BLOCK.METAL);
        }
      }
    }
    // Radiation exclusion sign poles: METAL 3-high + LIGHT top, every 12 blocks around perimeter
    for (i = -fw; i <= fw; i += 12) {
      setBlock(ox + i, h + 1, oz - fd + 2, BLOCK.METAL);
      setBlock(ox + i, h + 2, oz - fd + 2, BLOCK.METAL);
      setBlock(ox + i, h + 3, oz - fd + 2, BLOCK.METAL);
      setBlock(ox + i, h + 4, oz - fd + 2, BLOCK.LIGHT);
      setBlock(ox + i, h + 1, oz + fd - 2, BLOCK.METAL);
      setBlock(ox + i, h + 2, oz + fd - 2, BLOCK.METAL);
      setBlock(ox + i, h + 3, oz + fd - 2, BLOCK.METAL);
      setBlock(ox + i, h + 4, oz + fd - 2, BLOCK.LIGHT);
    ...
    for (i = -fd; i <= fd; i += 12) {
      setBlock(ox - fw + 2, h + 1, oz + i, BLOCK.METAL);
      setBlock(ox - fw + 2, h + 2, oz + i, BLOCK.METAL);
      setBlock(ox - fw + 2, h + 3, oz + i, BLOCK.METAL);
      setBlock(ox - fw + 2, h + 4, oz + i, BLOCK.LIGHT);
      setBlock(ox + fw - 2, h + 1, oz + i, BLOCK.METAL);
      setBlock(ox + fw - 2, h + 2, oz + i, BLOCK.METAL);
      setBlock(ox + fw - 2, h + 3, oz + i, BLOCK.METAL);
      setBlock(ox + fw - 2, h + 4, oz + i, BLOCK.LIGHT);
    }
    // Turbine hall: 30×12 CONCRETE 6 high on south side of reactors
    var thx = ox - 12, thz = oz + 25;
    var thh = getHeight(thx, thz);
    for (x = thx; x < thx + 30; x++) {
      for (z = thz; z < thz + 12; z++) {
        var isEdgeTH = (x === thx || x === thx + 29 || z === thz || z === thz + 11);
        if (isEdgeTH) {
          for (i = 0; i < 6; i++) setBlock(x, thh + i, z, BLOCK.CONCRETE);
        } else {
          setBlock(x, thh, z, BLOCK.CONCRETE);
        }
      }
    }
    // Turbine hall roof
    for (x = thx; x < thx + 30; x++) {
      for (z = thz; z < thz + 12; z++) {
        setBlock(x, thh + 6, z, BLOCK.CONCRETE);
      }
    }
    _buildings.push({ kind: 'landmark_zaporizhzhia_npp', x: ox - 35, z: oz - 22, w: 70, d: 45, baseY: h, floorH: 15, floors: 1, cx: ox, cz: oz });
  }

  // ── KRAMATORSK STATION: Missile Strike Memorial ───────────────────────────
  // On 8 April 2022, Russia fired a Tochka-U missile at Kramatorsk railway station,
  // killing 59 civilians (including 7 children) trying to evacuate Donbas.
  function generateKramatorskStation(ox, oz) {
    var h = getHeight(ox, oz);
    var x, z, i, j;
    // Main station building: 24×10 BRICK 8 high, decorative PLASTER arches, ROOFTILE roof
    for (x = ox - 12; x <= ox + 12; x++) {
      for (z = oz - 5; z <= oz + 5; z++) {
        var isEdgeSt = (x === ox - 12 || x === ox + 12 || z === oz - 5 || z === oz + 5);
        if (isEdgeSt) {
          for (i = 0; i < 8; i++) setBlock(x, h + i, z, BLOCK.BRICK);
        } else {
          setBlock(x, h, z, BLOCK.BRICK);
        }
      }
    }
    // ROOFTILE roof
    for (x = ox - 12; x <= ox + 12; x++) {
      for (z = oz - 5; z <= oz + 5; z++) {
        setBlock(x, h + 8, z, BLOCK.ROOFTILE);
      }
    }
    // Decorative PLASTER arches on facade every 4 blocks, 3 high
    for (x = ox - 12; x <= ox + 12; x += 4) {
      for (i = 0; i < 3; i++) {
        setBlock(x, h + i, oz - 5, BLOCK.PLASTER);
        setBlock(x, h + i, oz + 5, BLOCK.PLASTER);
      }
    }
    // Double-door entrance: 6-wide AIR gap at center of south facade
    for (x = ox - 3; x <= ox + 3; x++) {
      setBlock(x, h, oz - 5, BLOCK.AIR);
      setBlock(x, h + 1, oz - 5, BLOCK.AIR);
      setBlock(x, h + 2, oz - 5, BLOCK.AIR);
    }
    // 3 platform canopies: each 40×4 STONE with METAL support columns every 8 blocks, 5 high
    var pzOffsets = [oz + 10, oz + 16, oz + 22];
    for (j = 0; j < pzOffsets.length; j++) {
      var pz = pzOffsets[j];
      var ph = getHeight(ox, pz);
      // Platform base: 40×4 STONE slightly elevated
      for (x = ox - 20; x <= ox + 20; x++) {
        for (z = pz; z < pz + 4; z++) {
          setBlock(x, ph + 1, z, BLOCK.STONE);
        }
      }
      // METAL support columns every 8 blocks, 5 high
      for (x = ox - 20; x <= ox + 20; x += 8) {
        for (i = 0; i < 5; i++) {
          setBlock(x, ph + 2 + i, pz, BLOCK.METAL);
          setBlock(x, ph + 2 + i, pz + 3, BLOCK.METAL);
        }
      }
      // FENCE roof grid over platform
      for (x = ox - 20; x <= ox + 20; x++) {
        setBlock(x, ph + 7, pz, BLOCK.FENCE);
        setBlock(x, ph + 7, pz + 3, BLOCK.FENCE);
      }
    }
    // Railway tracks: STONE 2-wide strips running E-W, slightly elevated
    for (x = ox - 30; x <= ox + 30; x++) {
      setBlock(x, h + 1, oz + 12, BLOCK.STONE);
      setBlock(x, h + 1, oz + 13, BLOCK.STONE);
      setBlock(x, h + 1, oz + 18, BLOCK.STONE);
      setBlock(x, h + 1, oz + 19, BLOCK.STONE);
      setBlock(x, h + 1, oz + 24, BLOCK.STONE);
      setBlock(x, h + 1, oz + 25, BLOCK.STONE);
    }
    // Missile impact crater at (ox+5, oz+10): 8×8 AIR crater 3 deep, RUBBLE ring
    var crx = ox + 5, crz = oz + 10;
    var crh = getHeight(crx, crz);
    for (x = crx - 4; x <= crx + 4; x++) {
      for (z = crz - 4; z <= crz + 4; z++) {
        setBlock(x, crh, z, BLOCK.AIR);
        setBlock(x, crh - 1, z, BLOCK.AIR);
        setBlock(x, crh - 2, z, BLOCK.AIR);
      }
    }
    // RUBBLE ring around crater
    for (x = crx - 5; x <= crx + 5; x++) {
      setBlock(x, crh, crz - 5, BLOCK.RUBBLE);
      setBlock(x, crh, crz + 5, BLOCK.RUBBLE);
    }
    for (z = crz - 5; z <= crz + 5; z++) {
      setBlock(crx - 5, crh, z, BLOCK.RUBBLE);
      setBlock(crx + 5, crh, z, BLOCK.RUBBLE);
    }
    // Memorial: PLASTER cross 5 high + 3 wide at crater edge, STONE base
    var memx = crx - 7, memz = crz;
    var memh = getHeight(memx, memz);
    for (i = 0; i < 3; i++) setBlock(memx, memh + i, memz, BLOCK.STONE);
    for (i = 3; i < 8; i++) setBlock(memx, memh + i, memz, BLOCK.PLASTER);
    setBlock(memx - 1, memh + 5, memz, BLOCK.PLASTER);
    setBlock(memx + 1, memh + 5, memz, BLOCK.PLASTER);
    // Victim memorial candle clusters: WOOD base + LIGHT top, 8 scattered around crater
    var candlePos = [[-3,-6],[3,-6],[-6,-3],[6,-3],[-6,3],[6,3],[-3,6],[3,6]];
    for (j = 0; j < candlePos.length; j++) {
      var candx = crx + candlePos[j][0], candz = crz + candlePos[j][1];
      var candh = getHeight(candx, candz);
      setBlock(candx, candh + 1, candz, BLOCK.WOOD);
      setBlock(candx, candh + 2, candz, BLOCK.LIGHT);
    }
    // Burned luggage/debris: RUBBLE scattered in 6×6 area around crater
    var debrisPos = [[-2,-3],[2,-2],[-1,3],[3,1],[-3,2],[1,-3],[2,3],[-2,2]];
    for (j = 0; j < debrisPos.length; j++) {
      setBlock(crx + debrisPos[j][0], crh + 1, crz + debrisPos[j][1], BLOCK.RUBBLE);
    }
    // Destroyed evacuation buses: 2× CONCRETE blocks (8×3×2) with LIGHT emergency glow
    var busPositions = [[ox - 15, oz + 14],[ox + 10, oz + 14]];
    for (j = 0; j < busPositions.length; j++) {
      var bx = busPositions[j][0], bz2 = busPositions[j][1];
      var buH = getHeight(bx, bz2);
      for (x = bx; x < bx + 8; x++) {
        for (z = bz2; z < bz2 + 3; z++) {
          setBlock(x, buH + 1, z, BLOCK.CONCRETE);
          setBlock(x, buH + 2, z, BLOCK.CONCRETE);
        }
      }
      setBlock(bx, buH + 3, bz2, BLOCK.LIGHT);
      setBlock(bx + 7, buH + 3, bz2, BLOCK.LIGHT);
    }
    _buildings.push({ kind: 'landmark_kramatorsk_station', x: ox - 12, z: oz - 5, w: 24, d: 10, baseY: h, floorH: 8, floors: 1, cx: ox, cz: oz });
  }
