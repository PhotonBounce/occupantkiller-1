...
function fuelStorageTank(gx, gy, gz, scale) {
  ...
  /* ---- Stubs for missing functions called by voxel-world.js ---- */
  function playground(gx, gy, gz, scale) { for (var x = -2*scale; x <= 2*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) setBlock(gx+x, gy, gz+z, PAL.GRASS); setBlock(gx, gy+1, gz, PAL.METAL); }
  function cityHall(gx, gy, gz, scale) { for (var x = -3*scale; x <= 3*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) for (var y = 0; y < 4*scale; y++) { var wall = x===-3*scale||x===3*scale||z===-2*scale||z===2*scale||y===4*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.BRICK); } }
  function library(gx, gy, gz, scale) { for (var x = -2*scale; x <= 2*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) for (var y = 0; y < 3*scale; y++) { var wall = x===-2*scale||x===2*scale||z===-2*scale||z===2*scale; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.CONCRETE); } }
  function marketStall(gx, gy, gz, scale) { for (var x = -1*scale; x <= 1*scale; x++) for (var z = -1*scale; z <= 1*scale; z++) setBlock(gx+x, gy, gz+z, PAL.WOOD); for (var x = -1*scale; x <= 1*scale; x++) setBlock(gx+x, gy+1, gz, PAL.METAL); }
  function fireDepartment(gx, gy, gz, scale) { for (var x = -2*scale; x <= 2*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) for (var y = 0; y < 3*scale; y++) { var wall = x===-2*scale||x===2*scale||z===-2*scale||z===2*scale||y===3*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.BRICK); } setBlock(gx, gy+3*scale, gz, PAL.METAL); }
  function cafe(gx, gy, gz, scale) { for (var x = -1*scale; x <= 1*scale; x++) for (var z = -1*scale; z <= 1*scale; z++) for (var y = 0; y < 2*scale; y++) { var wall = x===-1*scale||x===1*scale||z===-1*scale||z===1*scale; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.WOOD); } }
  function hospitalBuilding(gx, gy, gz, scale) { for (var x = -3*scale; x <= 3*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) for (var y = 0; y < 5*scale; y++) { var wall = x===-3*scale||x===3*scale||z===-2*scale||z===2*scale||y===5*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.CONCRETE); } setBlock(gx, gy+1, gz+2*scale, PAL.GLASS); }
  function dataCenter(gx, gy, gz, scale) { for (var x = -2*scale; x <= 2*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) for (var y = 0; y < 3*scale; y++) { var wall = x===-2*scale||x===2*scale||z===-2*scale||z===2*scale||y===3*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.METAL); } }
  function gasStation(gx, gy, gz, scale) { for (var x = -1*scale; x <= 1*scale; x++) for (var z = -1*scale; z <= 1*scale; z++) setBlock(gx+x, gy, gz+z, PAL.ASPHALT); setBlock(gx, gy+1, gz, PAL.METAL); setBlock(gx+scale, gy+1, gz, PAL.METAL); }
  function recyclingCenter(gx, gy, gz, scale) { for (var x = -2*scale; x <= 2*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) for (var y = 0; y < 2*scale; y++) { var wall = x===-2*scale||x===2*scale||z===-2*scale||z===2*scale||y===2*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.METAL); } }
  function parkingLot(gx, gy, gz, scale) { for (var x = -3*scale; x <= 3*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) setBlock(gx+x, gy, gz+z, PAL.ASPHALT); }
  function waterTreatment(gx, gy, gz, scale) { for (var x = -2*scale; x <= 2*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) for (var y = 0; y < 2*scale; y++) { var wall = x===-2*scale||x===2*scale||z===-2*scale||z===2*scale; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.CONCRETE); } setBlock(gx, gy+2*scale, gz, PAL.METAL); }
  function concertHall(gx, gy, gz, scale) { for (var x = -3*scale; x <= 3*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) for (var y = 0; y < 4*scale; y++) { var wall = x===-3*scale||x===3*scale||z===-2*scale||z===2*scale||y===4*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.WHITE_TILE); } }
  function sportsComplex(gx, gy, gz, scale) { for (var x = -4*scale; x <= 4*scale; x++) for (var z = -3*scale; z <= 3*scale; z++) setBlock(gx+x, gy, gz+z, PAL.GRASS); for (var x = -2*scale; x <= 2*scale; x++) for (var z = -1*scale; z <= 1*scale; z++) setBlock(gx+x, gy, gz+z, PAL.ASPHALT); }
  function techPark(gx, gy, gz, scale) { for (var x = -2*scale; x <= 2*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) for (var y = 0; y < 3*scale; y++) { var wall = x===-2*scale||x===2*scale||z===-2*scale||z===2*scale||y===3*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.GLASS); } }
  function substation(gx, gy, gz, scale) { for (var x = -1*scale; x <= 1*scale; x++) for (var z = -1*scale; z <= 1*scale; z++) for (var y = 0; y < 2*scale; y++) { var wall = x===-1*scale||x===1*scale||z===-1*scale||z===1*scale; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.FENCE); } setBlock(gx, gy+2*scale, gz, PAL.METAL); }
  function trainStation(gx, gy, gz, scale) { for (var x = -3*scale; x <= 3*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) for (var y = 0; y < 3*scale; y++) { var wall = x===-3*scale||x===3*scale||z===-2*scale||z===2*scale||y===3*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.BRICK); } setBlock(gx, gy+1, gz+2*scale, PAL.AIR); setBlock(gx, gy+2, gz+2*scale, PAL.AIR); }
  function gymnasium(gx, gy, gz, scale) { for (var x = -3*scale; x <= 3*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) for (var y = 0; y < 3*scale; y++) { var wall = x===-3*scale||x===3*scale||z===-2*scale||z===2*scale||y===3*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.CONCRETE); } }
  function university(gx, gy, gz, scale) { for (var x = -4*scale; x <= 4*scale; x++) for (var z = -3*scale; z <= 3*scale; z++) for (var y = 0; y < 4*scale; y++) { var wall = x===-4*scale||x===4*scale||z===-3*scale||z===3*scale||y===4*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.BRICK); } }
  function apartmentBlock(gx, gy, gz, scale) { for (var x = -2*scale; x <= 2*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) for (var y = 0; y < 5*scale; y++) { var wall = x===-2*scale||x===2*scale||z===-2*scale||z===2*scale||y===5*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.CONCRETE); } }
  function bank(gx, gy, gz, scale) { for (var x = -2*scale; x <= 2*scale; x++) for (var z = -1*scale; z <= 1*scale; z++) for (var y = 0; y < 3*scale; y++) { var wall = x===-2*scale||x===2*scale||z===-1*scale||z===1*scale||y===3*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.STONE); } }
  function busShelter(gx, gy, gz, scale) { for (var x = -1*scale; x <= 1*scale; x++) for (var z = 0; z < 1*scale; z++) { setBlock(gx+x, gy, gz+z, PAL.GLASS); setBlock(gx+x, gy+1, gz+z, PAL.GLASS); } setBlock(gx-1*scale, gy, gz, PAL.METAL); setBlock(gx+1*scale, gy, gz, PAL.METAL); }
  function cinema(gx, gy, gz, scale) { for (var x = -2*scale; x <= 2*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) for (var y = 0; y < 3*scale; y++) { var wall = x===-2*scale||x===2*scale||z===-2*scale||z===2*scale||y===3*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.CONCRETE); } }
  function policeStation(gx, gy, gz, scale) { for (var x = -2*scale; x <= 2*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) for (var y = 0; y < 3*scale; y++) { var wall = x===-2*scale||x===2*scale||z===-2*scale||z===2*scale||y===3*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.BRICK); } setBlock(gx, gy+1, gz+2*scale, PAL.GLASS); }
  function museum(gx, gy, gz, scale) { for (var x = -3*scale; x <= 3*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) for (var y = 0; y < 3*scale; y++) { var wall = x===-3*scale||x===3*scale||z===-2*scale||z===2*scale||y===3*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.WHITE_TILE); } }
  function shoppingMall(gx, gy, gz, scale) { for (var x = -4*scale; x <= 4*scale; x++) for (var z = -3*scale; z <= 3*scale; z++) for (var y = 0; y < 3*scale; y++) { var wall = x===-4*scale||x===4*scale||z===-3*scale||z===3*scale||y===3*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.GLASS); } }
  function powerPlant(gx, gy, gz, scale) { for (var x = -3*scale; x <= 3*scale; x++) for (var z = -3*scale; z <= 3*scale; z++) for (var y = 0; y < 4*scale; y++) { var wall = x===-3*scale||x===3*scale||z===-3*scale||z===3*scale||y===4*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.METAL); } setBlock(gx, gy+4*scale, gz, PAL.SMOKE); }
  function sewagePlant(gx, gy, gz, scale) { for (var x = -2*scale; x <= 2*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) for (var y = 0; y < 2*scale; y++) { var wall = x===-2*scale||x===2*scale||z===-2*scale||z===2*scale; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.CONCRETE); } setBlock(gx, gy+2*scale, gz, PAL.METAL); }
  function researchLab(gx, gy, gz, scale) { for (var x = -2*scale; x <= 2*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) for (var y = 0; y < 3*scale; y++) { var wall = x===-2*scale||x===2*scale||z===-2*scale||z===2*scale||y===3*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.GLASS); } }
  function hotelBuilding(gx, gy, gz, scale) { for (var x = -2*scale; x <= 2*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) for (var y = 0; y < 4*scale; y++) { var wall = x===-2*scale||x===2*scale||z===-2*scale||z===2*scale||y===4*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.CONCRETE); } }
  function schoolBuilding(gx, gy, gz, scale) { for (var x = -3*scale; x <= 3*scale; x++) for (var z = -2*scale; z <= 2*scale; z++) for (var y = 0; y < 3*scale; y++) { var wall = x===-3*scale||x===3*scale||z===-2*scale||z===2*scale||y===3*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.BRICK); } }
  function postOffice(gx, gy, gz, scale) { for (var x = -1*scale; x <= 1*scale; x++) for (var z = -1*scale; z <= 1*scale; z++) for (var y = 0; y < 2*scale; y++) { var wall = x===-1*scale||x===1*scale||z===-1*scale||z===1*scale||y===2*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.BRICK); } }
  function waterTower(gx, gy, gz, scale) { for (var y = 0; y < 6*scale; y++) { setBlock(gx, gy+y, gz, PAL.METAL); } for (var x = -1*scale; x <= 1*scale; x++) for (var z = -1*scale; z <= 1*scale; z++) setBlock(gx+x, gy+6*scale, gz+z, PAL.CONCRETE); }
  function container(gx, gy, gz, scale) { for (var x = 0; x < 2*scale; x++) for (var z = 0; z < 1*scale; z++) for (var y = 0; y < 1*scale; y++) setBlock(gx+x, gy+y, gz+z, PAL.METAL); }
  function khrushchevka(gx, gy, gz, scale) { for (var x = -2*scale; x <= 2*scale; x++) for (var z = -1*scale; z <= 1*scale; z++) for (var y = 0; y < 5*scale; y++) { var wall = x===-2*scale||x===2*scale||z===-1*scale||z===1*scale||y===5*scale-1; if (wall) setBlock(gx+x, gy+y, gz+z, PAL.CONCRETE); } }
  function stoplight(gx, gy, gz) { setBlock(gx, gy, gz, PAL.METAL); setBlock(gx, gy+1, gz, PAL.LIGHT); }
  /* ---- End stubs ---- */
