...
window.VoxelWorld = (function () {
  ...
  // LANDMARK: Antonov Hangar (Hostomel, 2022) — giant aircraft hangar, partially collapsed
  // The Antonov An-225 Mriya (world's largest plane) was destroyed here 27 Feb 2022.
  // North half of hangar roof caved in from Russian artillery/airstrike. Blast craters around perimeter.
  function generateAntonovHangar(ox, oz) {
    ...
    for (var ci = 0; ci < craterSpots.length; ci++) {
      ...
      var crh = getTerrainHeight(crx2, crz2);
    ...
    // Runway strip (CONCRETE, 6 wide × 60 long, south of hangar)
  ...
  // LANDMARK: Kyiv Maidan Independence Monument (Berehynia column)
  // A 61m (real-world) white marble column on Freedom Square, crowned with a gilded winged Berehynia figure.
  // Inaugurated 23 Aug 2001. Became symbol of EuroMaidan revolution 2013–14. White Italian marble.
  function generateKyivMaidan(ox, oz) {
    var h = getTerrainHeight(ox, oz) || 0;
    var base = h;
    // Large plaza — WHITE_TILE cobblestone, 40×40
    for (var px = -20; px <= 19; px++) for (var pz = -20; pz <= 19; pz++) setBlock(ox + px, base, oz + pz, BLOCK.WHITE_TILE);
    // Steps up to plaza on all 4 sides (3 step tiers)
    for (var s = 0; s < 3; s++) {
      var sW = 20 - s;
    ...
    // Fountain basin ring around column (WATER, 3 blocks wide, 1 deep)
    ...
    // Central pedestal (3×3 STONE, 2 high)
    ...
    // Column shaft: 1×1 PLASTER, 20 blocks tall (represents the 61m real marble column)
    for (var col = 0; col < 20; col++) setBlock(ox, base + 3 + col, oz, BLOCK.PLASTER);
    // Berehynia figure at top: LIGHT orb (gilded) + wing/arm blocks (PLASTER)
    setBlock(ox, base + 23, oz, BLOCK.LIGHT);        // gilded figure / glow
    setBlock(ox - 1, base + 22, oz, BLOCK.PLASTER);  // left wing
    setBlock(ox + 1, base + 22, oz, BLOCK.PLASTER);  // right wing
    setBlock(ox, base + 22, oz - 1, BLOCK.PLASTER);  // forward arm
    // Lamp posts ringing the plaza (every 10 blocks along edges)
    ...
    for (var li = 0; li < lampPos.length; li++) {
      ...
      var ly = getTerrainHeight(lx, lz);
      setBlock(lx, ly + 1, lz, BLOCK.METAL);
      setBlock(lx, ly + 2, lz, BLOCK.METAL);
      setBlock(lx, ly + 3, lz, BLOCK.METAL);
      setBlock(lx, ly + 4, lz, BLOCK.LIGHT);
    }
    _buildings.push({ kind: 'landmark_kyiv_maidan', x: ox - 20, z: oz - 20, w: 40, d: 40, baseY: h, floorH: 24, floors: 1, cx: ox, cz: oz });
  }

  // LANDMARK: Khrushchyovka (Хрущівка) — Soviet 5-story prefab apartment block
  // Mass-produced 1955–1985, nicknamed after Khrushchev's housing programme.
  // Kharkiv Saltivka microdistrict had hundreds — most bombed residential area in Europe 2022.
  // Features: flat ROOFTILE roof, regular window grid, small balconies, CONCRETE + PLASTER texture mix.
  function generateKharkivApartments(ox, oz) {
    var h = getTerrainHeight(ox, oz) || 0;
    var base = h + 1;
    var bW = 16, bD = 6, bH = 5; // 5-story block
    // Walls: alternating CONCRETE and PLASTER for faded paint texture
    ...
    // Window grid: GLASS every 2 blocks horizontally, each floor (on north and south faces)
    ...
    // Balconies: every 2nd window has a METAL platform jutting 1 block out (south face)
    ...
    // Roof-top water tower (3-high cylinder of METAL, center of roof)
    ...
    // Entrance stairwell door (front face)
  ...
  function generateLevel(index) {
    ...
    if (level.id === 'HOSTOMEL') {
      ...
      // Antonov Hangar — the destroyed An-225 Mriya mega-hangar with collapsed north roof
      generateAntonovHangar(30, -20);           // NW apron: partially collapsed hangar (An-225 site)
      // Khrushchyovka apartments in Hostomel town (east of airport)
      generateKharkivApartments(-85, -40);      // Soviet-era 5-story block, west residential
      generateKharkivApartments(-85, -60);      // Second apartment block (pair)
      ...
      // Independence Monument (Berehynia) — 61m white marble column, gilded winged figure
      // Stands at the center of Maidan, symbol of Ukrainian independence and EuroMaidan 2014
      generateKyivMaidan(0, 5);                // Maidan center — Berehynia column + fountain plaza
