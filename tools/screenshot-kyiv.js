...
(async () => {
  ...
  try {
    ...
    // Build shot sequence: weapon IDs to cycle through for Kyiv (AT focus)
    const KYIV_WEAPONS = [7, 8, 10, 11, 12, 39, 40, 3, 5, 6]; // NLAW, Stugna, Javelin, RPG, Igla, M72LAW, Panzerfaust3, AK, SVD, PKM
    let shotCount = 0;
    let waveNum = 1;
    let weaponCycle = 0;
    ...
    while (shotCount < TARGET) {
      ...
      // Cycle weapons every 12 shots
      if (shotCount % 12 === 0) {
        const wIdx = KYIV_WEAPONS[weaponCycle % KYIV_WEAPONS.length];
        weaponCycle++;
        await p.evaluate((wi) => {
          try { if (wi < Weapons.getWeaponCount() && Weapons.isUnlocked(wi)) Weapons.switchTo(wi); } catch(e) {}
        }, wIdx);
        await sleep(200);
      }

      // Fire (simulate mousedown)
      await p.evaluate(() => {
        try {
          document.dispatchEvent(new MouseEvent('mousedown', { button: 0, bubbles: true }));
          setTimeout(() => document.dispatchEvent(new MouseEvent('mouseup', { button: 0, bubbles: true })), 120);
        } catch(e) {}
      });
      await sleep(80);
      ...
      if (shotCount % 50 === 0) {
        console.log(`  ${shotCount}/${TARGET} shots — wave ${waveNum}, ${aliveCount} enemies alive`);
      }
      await sleep(420);
    }
