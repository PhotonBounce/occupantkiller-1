...
const GameManager = (function () {
          ...
          if (typeof Bradley !== 'undefined' && Bradley.isActive && Bradley.isActive()) {
            ...
            if (!_bradleyMounted) {
              ...
              if (nearby.length > 0) {
                ...
                // (module exposes place(); it reads scene/camera from globals.
                //  There is no placeIED(), so the old call threw "not a function".)
  ...
  function onPlayerHit(dmg, attackerPos) {
    ...
    // (module exposes applyDamage(raw, hitLocation, hitDir) -> reduced damage;
    //  there is no absorbDamage(), so the old call threw "not a function" and
    //  aborted the rest of the hit handler.)
